    ...
    for( i=0;i<count;i++){
        for(j=0;j<count2;j++){
            if(strcmp(point[i].name,database[j])==0){
                for( k=point[i].start;k<point[i].end;k++){
                    database[j][k]=1;
            }
            }
        }
...
int main(int argc, const char * argv[]) {
    char path[200]="/rhome/xyhe/bigdata/dataxy/temp_res/mecat.ath";//三个参数的
    char path2[200]="/rhome/xyhe/bigdata/dataxy/temp_res/ref.ecoli";
