...
if __name__ == '__main__':
    ...
    for align in parser_tools.parse_align(src_align_path):
        # Count total base here
        total_base += align.query_len
    ...
    def thread_fun(read_name):
        ...
        if len(read_align) > 0:
            ...
            for aln in read_align:
                ...
                if hit_count / (aln.ref_end - aln.ref_begin) >= tp_cov:
                    max_aligned = max(max_aligned, aln.query_end - aln.query_begin)
            ...
            aligned = hit_count / (from_target.end - from_target.begin) >= fn_cov
    ...
    print()
    print('Aligned Base       = {}'.format(aligned_base))
